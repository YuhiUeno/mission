...
export async function findByHeroId(this: IHeroModel,
    min?: number,
    max?: number
): Promise<IHeroDocument[]> {
    return this.find({
        heroId: {$gte: min || 11, $lte: max || Infinity} }
    );
};
...
export async function getHero(this: IHeroModel,
    heroId: number
): Promise<IHeroDocument> {
...
export async function addHero(this: IHeroModel,
    name: string
): Promise<IHeroDocument> {
    ...
    await this.create({heroId: doc.seq, name: name});
    
    return await this.findOne({heroId: doc.seq})
};

export async function deleteHero(this: IHeroModel, heroId: number): Promise<IHeroDocument> {
